...
class PulpFacilitySimulation:
    ...
    def truck_arrival_process(self):
        ...
        while True:
            ...
            resultado_estrategia = self.strategy_func(truck, self)
            
            # --- NUEVO: Si la estrategia necesita tiempo para pensar (RL), la esperamos ---
            if isinstance(resultado_estrategia, types.GeneratorType):
                yield self.env.process(resultado_estrategia)
                
            # Ahora que ya sabemos la ruta, el camión entra a la planta
    ...
    def stock_monitoring_process(self):
        ...
        while True:
            ...
            # Verifica si 'algorithm' existe, si no está vacío, y si tiene '_CloseHour'
            if hasattr(self, 'algorithm') and hasattr(self.algorithm, '_CloseHour'):
                self.algorithm._CloseHour(
                    activeLines=len(self.lines),
                    remainingCraneVol=500.0,
                    maxCranePerHour=500.0,
                    ageStorageYard=ageStorageYard_days / 30.4167 
                )
